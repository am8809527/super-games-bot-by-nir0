...
module.exports = async (client, message) => {
  ...
  if (time_stamps.has(message.author.id)) {
    ...
    if (current_time < expiration_time) {
      ...
      return message.channel.send(new MessageEmbed().setColor(build.colors.err).setDescription(`**You Are In Cooldown Please Wait \`${time_left.toFixed(1)}\` To Use \`${command.name}\` Again**`))
    }
  }
